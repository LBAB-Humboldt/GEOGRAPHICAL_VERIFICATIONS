...
graficos=function(){
  ...
  points(ubicacion$longitud, ubicacion$latitud, col="red",cex=0.1, pch = 20)
  map(database = "world",add=T)
  mal.plot <- ubicacion[which(is.na(ubicacion$bienPais)),]
  points(mal.plot$longitud, mal.plot$latitud, col="black")
...
corroboracion <- function(datos,mun){
  ...
  if(length(which(!is.na(datos@data$municipio)))==1){
    ...
    #(MunCorrecto <- cbind(cntrm,as.character(datos@data$municipio))[jm,])
    ma1x <- NULL 
    uniqueMun <- unique(CompareMun[,2]) # Saco valores ?nicos por municipio
...
VERIFICACION_PAISES=function(ruta_salida,set3,ALT,mundo,colombia,casco,mpios,mpios2003,mpios1993,mpios1985,mpios1973,mpios1964,id,paises){
  ...
  if (ObjR=="YES"){
    cat("  2. Evaluando concordancia del pa?s (2 de 11)", "\n")
    print((Sys.time()))}
  ...
  sugerencia_pais2 <- paises@data$NAME_0[bien_pais]#
  set5$bienPais <- 0 #debug add
  set5$bienPais[which(set5$pais == sugerencia_pais)] <- 1#
  set5$bienPais[which(set5$pais == sugerencia_pais2)] <- 1#
  ...
  if (ObjR=="YES"){
    cat("  9. Evaluando registros para ?reas rurales y urbanas (9 de 11)", "\n")
    cat("     ",as.character(Sys.time()),"\n")}
  IGeor=set8 #
...
#   if (ObjR=="YES"){
#     set16 <<- cbind(set2[order(set2$ID),],set12[order(set12$id),-which((colnames(set16)%in%c("id","source","nombre","pais","departamento","municipio","latitud","longitud","fecha_inicial", "localidad")))])
#     set16 <- cbind(set2[order(set2$ID),],set12[order(set12$id),-which(colnames(set12)%in%c("id","source","nombre","pais","departamento","municipio","latitud","longitud","fecha_inicial", "localidad"))])
#     save(set16,file = paste0(ruta_salida,"/Registros_",as.Date(Sys.Date()),"_sinAlt.RData"))
#     col <- set16_sinAlt[which(set16_sinAlt$enCol ==1),c("species","speciesOriginal","bienPais","bien_depto","bien_muni","rural")]
#     nrow(col); nrow(set16)
#     resumen.salida <- rbind(
#       cbind(nrow(set16_sinAlt),nrow(col)),
#       cbind(length(which(set16_sinAlt$bienPais == 1)),length(which(col$bienPais == 1))),
#       cbind(length(which(set16_sinAlt$bienPais == 1 & set16_sinAlt$bien_depto == 1)),length(which(col$bienPais == 1 & col$bien_depto == 1))),
#       cbind(length(which(set16_sinAlt$bienPais == 1 & set16_sinAlt$bien_depto == 1 & set16_sinAlt$bien_muni == 1)),length(which(col$bienPais == 1 & col$bien_depto == 1 & col$bien_muni == 1))),
#       cbind(length(which(set16_sinAlt$bienPais == 1 & set16_sinAlt$bien_depto == 1 & set16_sinAlt$bien_muni == 1 & set16_sinAlt$rural==1)),length(which(col$bienPais == 1 & col$bien_depto == 1 & col$bien_muni == 1 & col$rural == 1)))
#     );resumen.salida
#     colnames(resumen.salida) <- c("Area de estudio", "Colombia")
#     rownames(resumen.salida) <- c("Total","Pais","Departamento","Municipio","Rural")
#     write.csv(resumen.salida, "06 Resumen_registros_sinAlt.csv")
#     rm(set16)
#     }
...
#     rownames(set12NA_NA) <- ncol(preset16)
#     names(set12NA_NA) <- names(set12)
#     preset16b=
#       rbind(preset16,set12NA_NA, deparse.level = 2)
#   }else{
#     preset16b=preset16  
#   }
  ...
  if (ObjR=="YES"){
    ...
    write.csv(resumen.salida, "Resumen/06 Resumen_salida.csv")
  }
}